...
# 暂时没用
def f_hour_weight():
    out_file = path_feature + hdf_hour_weight
    if util.is_exist(out_file):
        return
    ...
    # 存储
    util.safe_save(path_feature, hdf_hour_weight, hour_weight_df)
...
def f_click_count():
    """
    为 hdf_datatset 中的特征构造 click_count 特征，并存储到硬盘。
    """
    ...
    # 遍历数据集中的有效特征
    for c in (set(trainset_df.columns) & columns_set_to_construct_click_count):
        util.f_click_count(trainset_df, c)

    del trainset_df
    gc.collect()


def f_conversion_count():
    """
    为 hdf_datatset 中的特征构造 conversion_count 特征，并存储到硬盘。
    """
...
def fg_dataset(hdf_out, hdf_in):
    ...
    for c in (dataset_df.columns & columns_set_to_construct_conversion_ratio):
        ...
        # 添加特征
        dataset_df = util.add_feature(dataset_df, hdf_feature, f_conversion_ratio)
        # 填充缺失值
